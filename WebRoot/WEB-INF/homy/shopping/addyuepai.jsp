...
<html>
	<head>
		<meta charset="UTF-8">
		<title>预约发布</title>
		<link rel="stylesheet" type="text/css" href="../../../hanson/css/base.css"/>
		<style type="text/css">
			body{font-family: "微软雅黑";color: #333333;}
			*{box-sizing: border-box;}
			#content{background-color:#FFFFFF;}
			.ulNav{overflow: hidden;width: 100%;margin: 0px;margin-bottom: 10px;}
			.ulNav li{width: 33.33%;height: 50px;line-height: 50px;float: left;text-align: center;background-color: #F2F2F2;border: 1px solid #C9C9C9;}
			...
			.setPrice{float: left;margin: 0 10px;position: relative;}z
			.setPrice>div{
	...
	<body>
		...
		<form action="album/savebasicdata"  class="container-fluid form-horizontal" id="basicinfo" enctype="multipart/form-data" method="post" >
		<div id="essentialInformation">

			<div class="part1">基本信息</div>
			<div class="part2">
				<div><b>*</b><span>商品名称:</span><input type="text" id="projectname" name="projectname" placeholder="请输入商品名称" onblur="selectprojectname()"/></div>
				<div><b>*</b><span>商品英文名:</span><input type="text" id="englishname" name="englishname" placeholder="请输入商品英文名"/></div>
				<div><b>*</b><span>拍摄类型:</span>
					<select name="shootingtype" id="shootingtype">
						<option value="30">请选择</option>
						<option value="10">到门店拍</option>
						<option value="20">上门外拍</option>
					</select>
				</div>
				<div><b>*</b><span>现价:</span>
					<input type="text" name="totalamount" id="totalamount" placeholder="请输入现价"/>
				</div>
				<div><b>*</b><span>原价:</span>
					<input type="text" name="deposit" id="deposit" placeholder="请输入原价"/>
				</div>
				...
				<div><b>*</b><span>用途:</span>
					<textarea name="mainfunction" id="mainfunction" rows="10" cols="10"></textarea>
				</div>
				<div><b>*</b><span>增值服务:</span>
					<textarea name="addservice" id="addservice" rows="10" cols="10"></textarea>
				</div>
		...
		<div id="commodityPrice">
			<div class="part1">区域价格</div>
			<div class="part2">
				<div><b>*</b><span>地区:</span>
					<input type="text" placeholder="请输入市/区"/>
					<input type="text" placeholder="请输入区/县"/>
				...
				<div>
					<div class="setPrice">
						...
						<div><input type="number" min="0" name="" placeholder=""/><button class="confirmChange" data="price">确定</button></div>
					</div>
					<div class="setPrice">
						<button class="setBtn">一键设置原价</button>
						<div><input type="number" min="0" name="" placeholder=""/><button class="confirmChange" data="oldPrice">确定</button></div>
					</div>
					<div class="setPrice" style="display:none;">
						<button class="setBtn">一键设置加修加印价格</button>
						<div><input type="number" min="0" name="" placeholder=""/><button class="confirmChange" data="repairPrint">确定</button></div>
					</div>
					<div class="setPrice" style="display:none;">
						<button class="setBtn">一键设置加印价格</button>
						<div><input type="number" min="0" name="" placeholder=""/><button class="confirmChange" data="printPrice">确定</button></div>
					</div>
				</div>
				<div>
					<table class="price_t" border="" cellspacing="0" cellpadding="0">
						<thead>
							<tr><th>市/区</th><th>区/县</th><th>商户编号</th><th>公司名称</th><th>现价</th><th>原价</th></tr>
						</thead>
						<tbody>
							<tr><td>深圳市</td><td>罗湖区</td><td>0001</td><td>八卦岭店</td>
								<td><input type="number" min="0" value="99" class="price"/></td>
								<td><input type="number" min="0" value="198" class="oldPrice"/></td>
							</tr>
							<tr><td>深圳市</td><td>罗湖区</td><td>0001</td><td>八卦岭店</td>
								<td><input type="number" min="0" value="99" class="price"/></td>
								<td><input type="number" min="0" value="198" class="oldPrice"/></td>
							...
							<tr><td>深圳市</td><td>罗湖区</td><td>0001</td><td>八卦岭店</td>
								<td><input type="number" min="0" value="99" class="price"/></td><td>
								<input type="number" min="0" value="198" class="oldPrice"/></td>
							</tr>
							<tr><td>深圳市</td><td>罗湖区</td><td>0001</td><td>八卦岭店</td>
								<td><input type="number" min="0" value="99" class="price"/></td>
								<td><input type="number" min="0" value="198" class="oldPrice"/></td>
	...
	<script type="text/javascript">
		...
		/* 		if(parent=="essentialInformation"){
					p.hide(200);
					$("li[data-id='#"+parent+"']").removeClass("action");
					$("li[data-id='#commodityPrice']").addClass("action");
					$("#commodityPrice").show(200,function(){
						$(this).find(".part1").css("height",$(this).find(".part2").height());
					});
					...
					//	$(this).ajaxSubmit(options);
	  				//	return false; 
					//});
				}else if(parent=="commodityPrice"){
					p.hide(200);
					$("li[data-id='#"+parent+"']").removeClass("action");
					$("li[data-id='#productDetails']").addClass("action");
					$("#productDetails").show(200,function(){
						$(this).find(".part1").css("height",$(this).find(".part2").height());
					});

...
(function() {
	let $tempSel_1 = $(`<label class="pull-left pic-sel pic-wrapper" >
					    ...
					    <input name="templateImage" value="" type="hidden">
					</label>`);
	...
	let $tempSel = $(`<label class="pull-left pic-sel pic-wrapper" >
		...
		$wrapper1 = $("#template_scroll_wrapper");
	addNewTempSelScr();
	...
	function addNewTempSelScr() {
		...
		$wrapper1.append($clone1);
		$("#pro_scroll_wrapper label").each(function(index, item){
	    	addEventScr($(item));
		});
		$("#template_scroll_wrapper label").each(function(index, item){
	    	addEventScr($(item));
		});
	}
	
	// 添加新的sel, 并返回添加的新元素
	function addNewTempSelScr1(obj) {
		let $clone = $tempSel.clone();
		let $clone1 = $tempSel_1.clone();
		//判断当前操作的是对象是模板图还是录播图
...
function setTemplateImage(){
	$("#template_scroll_wrapper input[type='file']").each(function(index, item){
		$(this).attr("name", $(this).attr("name") + index);
	});
	$("#template_scroll_wrapper input[type='hidden']").each(function(index, item){
		$(this).attr("name", "templateImage" + index);
	});
}
...
function saveEditProduct(){
	...
	$("#template_scroll_wrapper input[type='hidden']:not(':last')").each(function(index, item){
		var str = $(this).val();
		if(str == ''){
			str = "null";
		}
		templateFileName = templateFileName + str + ",";
	});
	//赋值
	$("#templateImg").val(templateFileName);
	...
	$("#addProductForm").attr("action", "album/editProduct");
